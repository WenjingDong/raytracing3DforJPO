...
[rna,rnb,x,y,z,kx,ky,kz,t] = initialize(restart, ns, kh0, m0, Lx, nx, ny, nz, nsave);
...
x0 = x; y0 = y; z0 = z; kx0 = kx; ky0 = ky; kz0 = kz;
...
for ii=1:nt
    ...
    ltime = ltime + dt;
    [kx, ky, kz, x, y, z, ltime] = dissipate(f, N, mc, kx, ky, kz, x, y, z, ltime, kx0, ky0, kz0, x0, y0, z0);
...
function [rna,rnb,x,y,z,kx,ky,kz,t] = initialize(restart, ns, kh0, m0, Lx, nx, ny, nz, nsave)
     if restart == 1
        ...
        ltime = zeros(1,ns); % life time of a ray
	...
	fileID = fopen('dissipation.bin','w');
...
function [kx, ky, kz, x, y, z, ltime] = dissipate(f, N, mc,kx, ky, kz, x, y, z, ltime, kx0, ky0, kz0, x0, y0, z0)
    ind = abs(kz)>mc;
    if length(ind)>0
	    ltime_avg = sum(time(ind))/length(ind);
	    omegar = sqrt(N^1 * (kx.^2+ky.^2) + f^2 * kz.^2)....
	    /sqrt(kx.^2 + ky.^2 + kz.^2);
            fwrite(fileID,[length(ind), ltime_avg, omegar_avg]);

	    fileID = fopen('dissipation.bin','a');
            fwrite(fileID,[length(ind), ltime_avg, omegar_avg],'double');
	    fclose(fileID);
    end 
    kx(ind) = kx0(ind);
    ky(ind) = ky0(ind);
