...
dirname = '/scratch/wd583/raytracing3D/arrayjobs/Bu_charney/Buquarter_a01/';
folders = dir('/scratch/wd583/raytracing3D/arrayjobs/Bu_charney/Buquarter_a01/2f_run*');
nrun = length(folders);
...
omegar_2d = sqrt(N^2 * (k_2d.^2 + l_2d.^2) + f^2 * m_2d.^2)./K_2d;
...
grid on;
xlabel('$\omega/f$','interpreter','latex','fontsize',12);
ylabel('$e(\omega)$','interpreter','latex','fontsize',12);
tl = title('Time evolution of energy density $e(\omega)$');
...
subplot(1,2,2);
loglog(BinLoc, BinLoc.*mean(Count(:,ndata-20:ndata),2)); 
hold on;
grid on;
...
title(['slope: ' num2str(p(1),'%.5f')]);
%ylim([1e-3,1e1]);
xlim([1,N/f]);
set(gca,'xtick',[1:1:10 20:20:N/f]);
ylabel('Time averaged $e(\omega)$','interpreter','latex','fontsize',12);

save('data/Bu_charney/hist_Buquartera01.mat', 't', 'BinLoc', 'Count');
